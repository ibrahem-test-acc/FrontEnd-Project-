...
const observer = new IntersectionObserver((entries) => {
  entries.forEach((el) => {
    el.target.classList.toggle("numsSlider", el.isIntersecting);
  });
}, options);
...
window.onscroll = function () {
  ...
  if (window.scrollY < 276) {
    ...
    upBtn.classList.add("hUp");
    nav.style.backgroundColor = "#f5faff";
  } else if (window.scrollY >= 276) {
    upBtn.classList.remove("hUp");
    upBtn.classList.add("dUp");
    upBtn.style.display = "block";
    upBtn.style.cursor = "pointer";
    nav.style.position = "fixed";
    nav.classList.add("dNav");
...
menuIcon.onclick = function () {
  menu.classList.remove("slideX");
  menu.style.display = "block";
  menu.classList.toggle("slide");
  theNav.style.padding = "0";
  this.style.display = "none";
  xM.onclick = function () {
    menu.classList.toggle("slideX");
